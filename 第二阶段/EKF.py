...
A=np.onces((n, n), dtype= np.int) #A每次调用卡尔曼滤波后都会变
    ...
    for i in range(n):
        for j in range(n):
            if(A[i,j]==1):
                ...
                count=count+1
                Z_k[count,0]=z      # z为pnp输出的两机器人朝向的夹角的值，无法得出单独的朝向角度
    count=count+1
...
def robot_get1(X_old,z):
    for i in range(n):
        X_k[3*i+2,0]=X_old[3*i+2,0]+z[i,0]
    return  X_k

#直走D(一步预测)
def robot_get2(X_old,D):
    for i in range(n):
        X_k[3 * i + 0, 0] = X_old[3 * i + 0, 0] + D[i,0]*math.cos(X_old[3*i+2,0])
        X_k[3 * i + 1, 0] = X_old[3 * i + 1, 0] + D[i,0]*math.sin(X_old[3*i+2,0])
    return  X_k
...
def calcK_k1(P_now, R_k,X_old,z):
...
def calcK_k2(P_now, R_k,X_old,D):
...
def final_xz(X_k_position_xz,P_old_xz,Q_xz,R_k,X_old_xz,z):
    #先旋转的卡尔曼滤波
    Z_k_xz=calacZ_k(X_k_position_xz)
    P_now_xz=calcP_now(P_old_xz, Q_xz)
    K_k_xz=calcK_k1(P_now_xz, R_k,X_old_xz,z)
...
def final_zz(R_k,X_k_position_zz, P_old_zz, Q_zz, X_old_zz, D,final_Xk_xz):
    ...
    K_k_zz = calcK_k2(P_now_zz, R_k, X_old_zz, D)
    X_k_zz=robot_get2(final_Xk_xz, D)
    H_k_zz= calcH_k(X_k_position_zz)
