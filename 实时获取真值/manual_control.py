...
IP_LIST = ['192.168.121.248', '192.168.121.170']  # robomaster的IP列表
EP_DICT = {}
n = len(IP_LIST)  # 机器人个数
...
class EP:
    def __init__(self, ip):
        ...
        self.mac2ip = dict()
        self.ip2mac = dict()
        nano_ip = input("输入nano的IPV4地址：")
        mac = input("输入nano的MAC地址：")
        self.mac2ip[mac] = nano_ip
        self.ip2mac[nano_ip] = mac
        self.nano = pi_mgr(mac2ip, ip2mac)
    ...
    def get_now_position_landmark(self, k):
        # 世界坐标xx,yy,zz
        xx, yy, zz = 0, 1, 0
        # if master_numb == 1 or master_numb == 0:
        #     date = self.nano.get_output()
        #     now_POSITION[k, 0] = now_POSITION[] + date[3]
        #     now_POSITION[k + 1, 0] = yy + data[5]
        #     now_POSITION[k + 2, 0] = zz + 返回的角  # 观测得到的世界坐标
        date = self.nano.get_output()
        now_POSITION[k, 0] = xx + date[4]
        now_POSITION[k + 1, 0] = yy + data[5]
        now_POSITION[k + 2, 0] = zz + date[6]  # 观测得到的世界坐标
        world_pos = (now_POSITION[k, 0], now_POSITION[k + 1, 0], now_POSITION[k + 2, 0])

    def get_now_position_robo(self, k):
        # 两个机械人
        # 另一个机械人的坐标xx, yy, zz
        if k == 0:
            data = self.nano.get_output()
            now_POSITION[k, 0] = now_POSITION[k + 3, 0] + data[4]
            now_POSITION[k + 1, 0] = now_POSITION[k + 4, 0] + data[5]
            now_POSITION[k + 2, 0] = now_POSITION[k + 5, 0] + data[6]  # 观测得到的世界坐标
            world_pos = (now_POSITION[k, 0], now_POSITION[k + 1, 0], now_POSITION[k + 2, 0])
        else:
            data = self.nano.get_output()
            now_POSITION[k, 0] = now_POSITION[k - 3, 0] + data[4]
            now_POSITION[k + 1, 0] = now_POSITION[k - 2, 0] + data[5]
            now_POSITION[k + 2, 0] = now_POSITION[k - 1, 0] + data[6]  # 观测得到的世界坐标
            world_pos = (now_POSITION[k, 0], now_POSITION[k + 1, 0], now_POSITION[k + 2, 0])

    def get_true_position(self, k):
        data = self.nano.get_output()
        true[k, 0] = pos_start[k, 0] + data[0] * math.cos(
            pos_start[k + 2, 0] * math.pi / 180) - data[1] * math.sin(
            pos_start[k + 2, 0] * math.pi / 180)
        true[k + 1, 0] = pos_start[k + 1, 0] + data[0] * math.sin(
            pos_start[k + 2, 0] * math.pi / 180) + data[1] * math.cos(
            pos_start[k + 2, 0] * math.pi / 180)
        true[k + 2, 0] = pos_start[k + 2, 0] + data[2]
...
if __name__ == "__main__":
    ...
    for ip in IP_LIST:
        ...
        EP_DICT[ip] = EP(ip)
        EP_DICT[ip].nano.launch()
        EP_DICT[ip].start()
        EP_DICT[ip].inStart_pos(a)
        visualization.putstart(IP_LIST,[pos_start[a,0],pos_start[a+1,0],pos_start[a+2,0]])
        a = a + 3
    ...
    while p < 2:
        ...
        for ip in IP_LIST:
            # if 看到landmark:
            if data[-1] == 16:
                EP_DICT[ip].get_now_position_landmark(k)
            else:
                EP_DICT[ip].get_now_position(k)
            EP_DICT[ip].get_true_position(k)
            # new_add
            data = EP_DICT[ip].nano.get_output()
            master_numb = data[-1]
            if master_numb == 1:
                EKF.A[0, 1] = 1
            elif master_numb == 0:
        ...
        for ip in IP_LIST:
            ...
            if data[-1] == 16:
                EP_DICT[ip].get_now_position_landmark(k)
            # elif 看到robo:
            elif data[-1] == 1 or data[-1] == 0:
                EP_DICT[ip].get_now_position_robo(k)
            k = k + 3
        b = b + 1
